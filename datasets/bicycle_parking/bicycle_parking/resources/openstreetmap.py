...
def feature_from_point_element(element: dict) -> dict:
    ...
    if "lon" in element and "lat" in element:
        ...
        raise KeyError(
            f"""Unable to find coordinates ("lon", "lat", or "center") in element {element["type"] + "/" + str(element["id"])}"""
        )

    meta_properties = {
        "meta_" + key: value for (key, value) in element.items() if key in INCLUDE_META
    }
    # convert "Z" suffix to "+00:00"
    timestamp = {
        "meta_timestamp": datetime.fromisoformat(element["timestamp"]).isoformat(),
    }
    view_url = {"meta_url_nwr": VIEW_URL + element["type"] + "/" + str(element["id"])}
